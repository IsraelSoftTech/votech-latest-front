...
export default function Admin2App({ authUser }) {
  ...
  useEffect(() => {
    ...
    api.getAllTeachers().then(all => {
      ...
      let userApps = [];
      // Try multiple ways to match the user, prioritizing user_id
      if (authUser?.id) {
        userApps = all.filter(t => t.user_id === authUser.id);
        console.log('Found by user_id:', userApps);
      }
      
      // If no apps found by user_id, try other methods
