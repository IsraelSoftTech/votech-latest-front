...
export default function AdminTeacher() {
  ...
  useEffect(() => {
    ...
    console.log('[DEBUG] Filtering approved applications:', {
      ...
      allStatuses: applications.map(app => ({ id: app.id, status: app.status, name: app.applicant_name }))
    });
  ...
  useEffect(() => {
    ...
    setApproveStates(Object.fromEntries(applications.map(a => [a.id, a.status])));
  }, [applications]);

  const fetchApplications = async () => {
    try {
      console.log('[DEBUG] Fetching applications for user:', authUser?.username, 'Role:', authUser?.role);
      ...
      <div className="teacher-section">
          ...
          <table className="teacher-table">
            ...
            <tbody>
              {approvedStaff.map((staff, i) => (
                <tr key={staff.id || i}>
                  <td>{i + 1}</td>
                  <td className="staff-name">{staff.applicant_name || staff.applicant_full_name}</td>
                  <td>{staff.applicant_username}</td>
                  <td>{staff.subjects}</td>
                  <td>{staff.classes ? staff.classes.split(',').filter(c => c.trim() !== 'undefined').join(', ') : ''}</td>
                  <td>{staff.contact}</td>
                  <td>{staff.submitted_at ? new Date(staff.submitted_at).toLocaleDateString() : ''}</td>
                  <td>
                    {staff.certificate_url ? (
                      <button className="action-btn view-certificate" title="View Certificate" onClick={() => handleViewCertificate(staff)}><FaEye /></button>
                    ) : (
        ...
        {approvedStaff.length === 0 && (
          <div className="empty-state">
            ...
            <p>There are no approved staff members yet. Staff members will appear here once their applications are approved by Admin4.</p>
          </div>
        )}
      </div>
          ...
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '90vw', maxHeight: '90vh', width: 'auto', height: 'auto' }}>
            ...
            <div style={{ padding: '20px 0', textAlign: 'center' }}>
              <h3 style={{ color: '#204080', marginBottom: 16, fontSize: 20 }}>
                Certificate - {selectedCertificate.applicant_name || selectedCertificate.applicant_full_name}
              </h3>
